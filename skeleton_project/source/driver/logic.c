...
void chooseDirection(state* currentState, int* currentDirection) {
    if (numberOfOrders() == 1){
        for (int i = 0; i < MAX_ORDERS; i++) {
            ...
            else if (((g_order[i][0] > g_lastDefinedFloor) && (g_order[i][0] != -1))|| ((g_order[i][0] == g_lastDefinedFloor) && (*currentDirection == DOWN))) {
        ...
        for (int i = 0; i < MAX_ORDERS; i++) {
            if ((g_order[i][0] > g_lastDefinedFloor) && (g_currentDirection == UP))
            {
                elevio_motorDirection(DIRN_UP);
                *currentState = MOVING;
            ...
            else if ((g_order[i][0] < g_lastDefinedFloor) && (g_currentDirection == DOWN))
            {
                elevio_motorDirection(DIRN_DOWN);
                *currentState = MOVING;
